...
function getMostRecentThursday(): Date {
  ...
  lastThursday.setHours(20, 30, 0, 0); // 8:30pm
  return lastThursday;
}

function getNextThursdayMidnight(lastThursday: Date): Date {
...
export default function LastThursdayLeaderboard({
  searchedUsername,
}: {
  ...
  useEffect(() => {
    const now = new Date();
    const lastThursday = getMostRecentThursday();
    const nextThursdayMidnight = getNextThursdayMidnight(lastThursday);

    if (now >= lastThursday && now < nextThursdayMidnight) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-bold text-center text-white">
        Scores from Thursday {quizDateString}
      </h2>

...
function SignInForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // If user typed a username, resolve it to an email via RPC
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      {/* NEW: Resend verification UI (lives in SignIn) */}
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const { error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          data: { username }, // <-- goes to raw_user_meta_data.username for the trigger
          emailRedirectTo: `${location.origin}/auth/callback`,
        },
      });
      if (error) throw error;
      router.push("/auth/check-email"); // or wherever your "verify email" screen lives
    } catch (err: any) {
